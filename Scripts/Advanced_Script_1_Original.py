...
feature_cols = (
    ["Union_Gap_UD", "Mabton_UD", "month", "dayofyear"] +
...
mod_mab = RandomForestRegressor(n_estimators=100, max_depth=20, random_state=42, n_jobs=-1)
mod_kio = RandomForestRegressor(n_estimators=100, max_depth=20, random_state=42, n_jobs=-1)
...
def forecast_gage_heights(flows, start_time, init_history, steps=192, alpha=0.05):
    """
    ...
    time = start_time + pd.Timedelta(days=14.15)
    # Edit "days = #" to reflect actual date, assuming initial value is Jan 1
    # Edit "hours = #" to reflect actual time of day, assuming initial value is 8 AM
...
if __name__ == "__main__":
    ...
    # Build initial history from last LAGS rows in training dataframe
    init = {
    ...
    start_time = df["time"].iloc[-1] + pd.Timedelta(minutes=15)

    Xrow = make_feature_row(init, (ug_flow, mb_flow), start_time)
    print(prediction_interval(mod_kio, Xrow))

    fc = forecast_gage_heights(
        flows=(ug_flow, mb_flow),
        start_time=start_time,
        init_history=init,
        steps=192  # = 48 hours at 15-minute resolution
    )
