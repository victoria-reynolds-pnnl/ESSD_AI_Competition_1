...
df = df.drop_duplicates(subset=["monitoring_location_id", "day_idx"])
...
g = df.groupby("monitoring_location_id", sort=False)[["day_idx", "qualifier"]]
...
for loc, idxs_and_qualifiers in g:
    idxs = idxs_and_qualifiers['day_idx']
    qualifiers = idxs_and_qualifiers['qualifier']
    r = loc_to_row[loc]
    present[r, idxs.to_numpy(dtype=np.int32, copy=False)] = 1
    present[r, idxs[qualifiers.notna()]] = 3
    present[r, idxs[qualifiers.apply(qualifiers_acceptable)]] = 2
    present[r, idxs[qualifiers.isna()]] = 1
...
cmap = mpl.colors.ListedColormap([
    'white', # Data is absent
    'black', # Data is present
    'green', # 'Estimated' or 'Revised' qualifier
    'red',   # other qualifiers
    ])
ax.imshow(
    present, 
    aspect="auto", 
