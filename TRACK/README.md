...
We trained and evaluated two binary classification models—Logistic Regression and XGBoost—to identify high-risk extreme weather records. The classification target was derived from HDBSCAN-based 3-tier risk labeling and then collapsed into a binary `high_risk` outcome for Week 3. We used F1 and AUC-ROC because this is a classification task, and those are the recommended metrics for classification problems [2]. F1 is useful because it balances precision and recall when identifying high-risk cases, while AUC-ROC measures how well the model separates high-risk from non-high-risk events across thresholds [2].
