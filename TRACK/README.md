...
*   **`data/cleaned/`**: This folder will be created by the script if it doesn't exist. It will store the cleaned output files.
*   **`Scripts/`**: This folder must contain the `clean_data.py` script and the `requirements.txt` file.

### Setup and Execution

1.  **Place Files**:
    *   Move all the provided `.zip` data archives into the `data/original/` directory.
    *   Ensure `clean_data.py` and `requirements.txt` are located in the `Scripts/` directory.
...
After the script finishes, the cleaned data will be located in the `data/cleaned/` directory. The output is organized into subdirectories named after the original `.zip` archives.
